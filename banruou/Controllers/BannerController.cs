        ...
        public ActionResult Banner(BannerViewModel model, FormCollection fc)
        {
            if (ModelState.IsValid)
            {
                ...
                if (file != null && file.ContentLength > 0)
                {
                    ...
                    {
                        if (file.ContentLength > 150000 * 1024)
                        {
                            ModelState.AddModelError("", @"Dung lượng lớn hơn 150MB. Hãy thử lại");
                            isPost = false;
                        }
                        else
                        {
                ...
                if (isPost)
                {
                    model.Banner.Url = HtmlHelpers.ConvertToUnSign(null, model.Banner.Url ?? model.Banner.BannerName);
                    var url = _unitOfWork.BannerRepository.GetQuery(a => a.Url == model.Banner.Url);
                    if(url != null)
                    {
                        model.Banner.Url += "-" + DateTime.Now.Millisecond;
                        _unitOfWork.Save(); 
                    }
        ...
        public ActionResult EditBanner(BannerViewModel model, FormCollection fc)
        {
            if (ModelState.IsValid)
            {
                ...
                if (isPost)
                {
                    ...
                    banner.Url = HtmlHelpers.ConvertToUnSign(null, model.Banner.Url ?? model.Banner.BannerName);
                    var url = _unitOfWork.BannerRepository.GetQuery(a => a.Url == banner.Url);
                    if (url != null)
                    {
                        banner.Url += "-" + DateTime.Now.Millisecond;
                        _unitOfWork.Save();
                    }
